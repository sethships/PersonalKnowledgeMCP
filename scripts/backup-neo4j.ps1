<#
.SYNOPSIS
    Neo4j Volume Backup Script for Windows PowerShell
...
.EXAMPLE
    .\backup-neo4j.ps1 -BackupDir "D:\Backups" -RetentionDays 7
    Creates backup in D:\Backups, keeping only last 7 days of backups.

.EXAMPLE
    .\backup-neo4j.ps1 -Quiet
...
# Script state
$script:ContainerWasRunning = $false
...
function Stop-Neo4jContainer {
    ...
    $script:ContainerWasRunning = $true
    Write-Info "Stopping Neo4j container(s) for consistent backup: $($containers -join ', ')"

    if ($DryRun) {
        Write-Info "[DRY-RUN] Would stop Neo4j container(s)"
        return
    }
...
function Start-Neo4jContainer {
    if (-not $script:ContainerWasRunning) {
        return
    }

    Write-Info "Restarting Neo4j container..."

    if ($DryRun) {
        Write-Info "[DRY-RUN] Would restart Neo4j container"
        return
    }

    $result = docker compose up -d neo4j 2>$null
    if ($LASTEXITCODE -eq 0) {
        Write-Info "Container restarted via docker compose"
        return
    }

    Write-Warn "Could not restart container automatically"
...
function New-Backup {
    ...
    $backupFile = "neo4j-backup-$timestamp.tar.gz"
...
function Remove-OldBackups {
    ...
    $oldBackups = Get-ChildItem -Path $BackupPath -Filter "neo4j-backup-*.tar.gz" -File |
...
function Show-ExistingBackups {
    ...
    $backups = Get-ChildItem -Path $BackupPath -Filter "neo4j-backup-*.tar.gz" -File -ErrorAction SilentlyContinue |
...
function Main {
    Write-Info "Neo4j Backup Script (PowerShell)"
    Write-Info "================================="
