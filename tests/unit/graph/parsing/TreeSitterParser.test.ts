...
`;
  ...
  describe("parseFile - Go Functions", () => {
    it("should parse simple Go functions", async () => {
      const content = await Bun.file(path.join(FIXTURES_DIR, "simple-function.go")).text();
      const result = await parser.parseFile(content, "simple-function.go");
      ...
      expect(addFunc?.isExported).toBe(true); // Uppercase = exported in Go
      ...
      expect(greetFunc?.metadata?.returnType).toBe("string");

      // Find the private processItems function (lowercase = private)
      const privateFunc = result.entities.find((e) => e.name === "processItems");
      expect(privateFunc).toBeDefined();
      expect(privateFunc?.isExported).toBe(false);
    });

    it("should parse functions with variadic parameters", async () => {
      const content = await Bun.file(path.join(FIXTURES_DIR, "simple-function.go")).text();
      const result = await parser.parseFile(content, "simple-function.go");

      const formatFunc = result.entities.find((e) => e.name === "FormatMessage");
      expect(formatFunc).toBeDefined();
      expect(formatFunc?.metadata?.parameters).toBeDefined();
      // Should have variadic args parameter
      const argsParam = formatFunc?.metadata?.parameters?.find((p) => p.isRest);
      expect(argsParam).toBeDefined();
    });

    it("should parse functions with multiple return values", async () => {
      const content = await Bun.file(path.join(FIXTURES_DIR, "simple-function.go")).text();
      const result = await parser.parseFile(content, "simple-function.go");

      const multiReturnFunc = result.entities.find((e) => e.name === "MultiReturn");
      expect(multiReturnFunc).toBeDefined();
      expect(multiReturnFunc?.metadata?.returnType).toBeDefined();
      // Should capture the multiple return values
      expect(multiReturnFunc?.metadata?.returnType).toContain("int");
      expect(multiReturnFunc?.metadata?.returnType).toContain("error");
    });

    it("should extract Go doc comments as documentation", async () => {
      const content = await Bun.file(path.join(FIXTURES_DIR, "simple-function.go")).text();
      const result = await parser.parseFile(content, "simple-function.go");

      const addFunc = result.entities.find((e) => e.name === "Add");
      expect(addFunc?.metadata?.documentation).toBeDefined();
      expect(addFunc?.metadata?.documentation).toContain("Add adds two integers");
    });
  });

  describe("parseFile - Go Structs and Methods", () => {
    it("should parse Go structs as classes", async () => {
      const content = await Bun.file(path.join(FIXTURES_DIR, "complex-struct.go")).text();
      const result = await parser.parseFile(content, "complex-struct.go");

      expect(result.success).toBe(true);

      // Find Handler struct (should be parsed as class type)
      const handler = result.entities.find((e) => e.name === "Handler");
      expect(handler).toBeDefined();
      expect(handler?.type).toBe("class");
    ...
    it("should parse Go interfaces", async () => {
      ...
      // Find DataProcessor interface (parsed as class type)
      const dataProcessor = result.entities.find((e) => e.name === "DataProcessor");
      expect(dataProcessor).toBeDefined();
      expect(dataProcessor?.type).toBe("class");
      expect(dataProcessor?.isExported).toBe(true);
    });
    ...
    it("should parse Go methods with receivers", async () => {
      ...
      // Find Process method
      const processMethod = result.entities.find(
        (e) => e.name === "Process" && e.type === "method"
      );
      expect(processMethod).toBeDefined();
      expect(processMethod?.type).toBe("method");
      expect(processMethod?.isExported).toBe(true);
      // Receiver should be captured in metadata.extends
      expect(processMethod?.metadata?.extends).toBeDefined();

      // Find private method
      const doSomething = result.entities.find((e) => e.name === "doSomething");
      expect(doSomething).toBeDefined();
      expect(doSomething?.type).toBe("method");
      expect(doSomething?.isExported).toBe(false);
    });

    it("should parse generic Go types (Go 1.18+)", async () => {
      const content = await Bun.file(path.join(FIXTURES_DIR, "complex-struct.go")).text();
      const result = await parser.parseFile(content, "complex-struct.go");

      // Find GenericContainer
      const genericContainer = result.entities.find((e) => e.name === "GenericContainer");
      expect(genericContainer).toBeDefined();
      expect(genericContainer?.type).toBe("class");
    });
  });

  describe("parseFile - Go Imports", () => {
    it("should parse Go import statements", async () => {
      const content = await Bun.file(path.join(FIXTURES_DIR, "go-imports.go")).text();
      const result = await parser.parseFile(content, "go-imports.go");
      ...
      const jsonImport = result.imports.find((i) => i.source === "encoding/json");
      expect(jsonImport).toBeDefined();

      // Check for aliased import
      const ctxImport = result.imports.find(
        (i) => i.source === "context" && i.aliases?.["context"] === "ctx"
      );
      expect(ctxImport).toBeDefined();

      // Check for blank import (side effect)
      const driverImport = result.imports.find((i) => i.source === "database/sql/driver");
      expect(driverImport).toBeDefined();
      expect(driverImport?.isSideEffect).toBe(true);
    });
  });
  ...
  describe("parseFile - Go Function Calls", () => {
    it("should parse Go function calls", async () => {
      ...
      // Find fmt.Sprintf call
      const sprintfCall = result.calls.find((c) => c.calledName === "Sprintf");
      expect(sprintfCall).toBeDefined();
      expect(sprintfCall?.calledExpression).toContain("fmt");
    });

    it("should track caller context for Go functions", async () => {
      const content = await Bun.file(path.join(FIXTURES_DIR, "simple-function.go")).text();
      const result = await parser.parseFile(content, "simple-function.go");

      // Find calls inside the Greet function
      const callsInGreet = result.calls.filter((c) => c.callerName === "Greet");
      expect(callsInGreet.length).toBeGreaterThan(0);
    });

    it("should parse method calls in Go", async () => {
      const content = await Bun.file(path.join(FIXTURES_DIR, "complex-struct.go")).text();
      const result = await parser.parseFile(content, "complex-struct.go");

      // Find Lock call in Process method
      const lockCall = result.calls.find(
        (c) => c.calledName === "Lock" && c.callerName === "Process"
      );
  ...
  describe("parseFile - Go Exports", () => {
    it("should return empty exports for Go (exports determined by capitalization)", async () => {
      const content = await Bun.file(path.join(FIXTURES_DIR, "simple-function.go")).text();
      const result = await parser.parseFile(content, "simple-function.go");

      // Go doesn't have explicit export statements
      // Exports are determined by capitalization (handled in entity extraction)
      expect(result.exports).toHaveLength(0);
    });
  });

  describe("static methods - Go support", () => {
    it("should correctly identify Go extensions", () => {
